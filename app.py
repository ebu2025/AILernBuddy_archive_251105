...
async def _lifespan(_: FastAPI):
    ...
    print("OpenAI-style params in use:", _base_params(), "| SEND_MAX_TOKENS:", SEND_MAX_TOKENS)
    _ensure_rag_ready()
    yield
...
_PROGRESSION_ENGINE = ProgressionEngine(window_size=5, min_attempts=3)
...
def _hash_pw(password: str) -> str:
    salt = os.getenv("AUTH_SALT", "local_salt")
    return hmac.new(salt.encode(), password.encode(), digestmod="sha256").hexdigest()
...
def rag_query(body: RagQueryBody):
    ...
    try:
        ...
        _RAG_LOGGER.warning("RAG query failed: %s", exc)
...
def auth_register(body: RegisterBody):
    ...
    db.create_user(body.user_id, email, pw_hash)
    return {"ok": True}

@app.post("/auth/login")
def auth_login(body: LoginBody):
    row = db.get_user_auth(body.user_id)
    if not row or row["pw_hash"] != _hash_pw(body.password):
        ...
        try:
            ...
            if attempt == 0:
                ...
                retry_context_parts.append(
                    "Your previous reply failed JSON schema validation. Provide the teaching reply followed by a single JSON object that matches the required tutor schema."
                    f" Error: {exc}"
                )
