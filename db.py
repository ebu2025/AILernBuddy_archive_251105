...
def get_user_auth(user_id: str) -> Optional[sqlite3.Row]:
    rows = _query("SELECT user_id, pw_hash FROM users WHERE user_id = ?", (user_id,))
...
def create_user(user_id: str, email: Optional[str], pw_hash: str):
    _exec("INSERT INTO users(user_id, email, pw_hash) VALUES (?,?,?)", (user_id, email, pw_hash))

def ensure_user(user_id: str):
    rows = _query("SELECT 1 FROM users WHERE user_id = ?", (user_id,))
    if not rows:
        # Minimal setup without email/password (for guests, etc.)
        _exec("INSERT INTO users(user_id, email, pw_hash) VALUES (?,?,?)", (user_id, f"{user_id}@local", "NA"))
