...
@dataclass
class VectorStore:
    """Abstraction over Chroma or an in-memory fallback."""
    ...
    def query(self, query: str, k: int = 4) -> List[RetrievedChunk]:
        embedding = self.embedding_backend.embed(query)
        ...
        if self.collection is not None:
            ...
            for idx, doc in enumerate(docs):
                ...
                retrieved.append(
                    RetrievedChunk(
                        id=ids[idx] if idx < len(ids) else f"result-{idx}",
                        content=doc,
                        metadata=metadata,
                        score=score,
                    )
        ...
        for chunk, stored_embedding in self._fallback_store:
